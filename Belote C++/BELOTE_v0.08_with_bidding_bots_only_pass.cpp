...
const int deck = 32;
char deckStyle=178;
int pos1=0,pos2=0,pos3=0,pos4=0,bid=0;
bool f=0;
int passNumber=0,whoCalled;
void makeDeck(int a[]){
    int br1 = 0, def = 107;
    for(int i = 0; i<32 ;i++){
        a[i] = def ;
        ...
        if(i%4==0)
            {
                ...
                for(int j=0;j<4;j++)
                {
                    ...
                    if(bid%10<5&&highestCardOnBoard!=bid%10&&cardsOnBoard[j]/100==bid%10&&cardsOnBoard[j]/100>highestTrumpOnTheBoard)
                        {
                            whoGotIt=j;
...
void playerBid(int p[],int whoAmI,int p1[]){
    ...
    if(whoAmI==0){
        ...
        for(int i=1;i<8;i++)
        {
            ...
            case 7:cout<<"7 for Pass"<<endl;break;
            }
        }
        cin>>playerbid;
        while(playerbid<0||playerbid>7||playerbid<=bid%10){
...
int main(){
    ...
    for(int i=0;realScore[0]<151&&realScore[1]<151;i++){
        bid=0;
        makeDeck(theDeck);
        shuffleDeck(theDeck);
