...
const int deck = 32;
char deckStyle=178;
int pos1=0,pos2=0,pos3=0,pos4=0,bid=0,redouble=0,anno[2]={0,0},hang=0,passNumber=0,bids[18],bidsPosition=0,whoCalledTheDouble=5,whoCalledTheReDouble=5;
    ...
    if(redouble==1&&capot==0)
    {
        ...
        if(score[bidderTeam]<score[nonbidderTeam])
        ...
        else
            {
                ...
                score[bidderTeam]=round(score[bidderTeam]);
                score[bidderTeam]*=2;
...
void bidding(int p1[],int p2[],int p3[],int p4[],int plOnTheGo){
    ...
    switch(potg){
        ...
        {potg=0;}
        if((whoPlayedFirst==potg||whoCalledTheDouble==potg||whoCalledTheReDouble==potg)&&passNumber==3)
        break;
    }
}
int main(){
    ...
    for(int k=0;(realScore[0]<151&&realScore[1]<151)||(capot==1);k++){
        ...
        for(int i=8;i<18;i++)
            bids[i]=0;
        bidsPosition=0;
        whoCalledTheDouble=5;
        whoCalledTheReDouble=5;
        makeDeck(theDeck);
        shuffleDeck(theDeck);
        ...
        if(whoCalledTheDouble!=5) cout<<"Player "<<whoCalledTheDouble+1<<" called Double."<<endl;
        if(whoCalledTheReDouble!=5) cout<<"Player "<<whoCalledTheReDouble+1<<" called Re Double."<<endl;
        Sleep(2000);
