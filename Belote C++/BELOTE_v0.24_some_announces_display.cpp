...
bool f=0,f2=0,wasThereHanging=0,test=1;
bool capot=0;
void makeDeck(int a[]){
    int br1 = 0, def = 107;
    for(int i = 0; i<32 ;i++){
...
for(int i=0;i<8;i++)
    ...
    for(int i=0;i<7;i++)
    {
        if(p1[i]%100==7||p1[i]%100==8||p1[i]%100==9)
            continue;
        for(int j=i+1;j<8;j++)
        {
...
void view(int p1[],int p2[],int p3[],int p4[],int cob[], int potg){
        ...
        for(int i=0;i<80;i++)
            {
                    ...
                    if(j==4&&i==45&&potg==1&&playerAnnounce(p2)!="")
                    {
                        f=1;
                    ...
                    if(j==4&&i==3&&potg==3&&playerAnnounce(p4)!="")
                    {
                        f=1;
...
void playTheGame(int playerOne[],int playerTwo[],int playerThree[],int playerFour[],int realScore[],int plOnTheGo ){
        ...
        view(playerOne,playerTwo,playerThree,playerFour,cardsOnBoard,4);
        anno[0]=0;
        anno[1]=0;
        if(bid%10!=5){
        ...
        system("cls");
        if(i<5)
            view(playerOne,playerTwo,playerThree,playerFour,cardsOnBoard,plOnTheGo);
        else
            view(playerOne,playerTwo,playerThree,playerFour,cardsOnBoard,4);
        if(plOnTheGo==3)
            plOnTheGo=0;
        ...
        if(score[bidderTeam]<score[nonbidderTeam]){
                score[bidderTeam]-=anno[bidderTeam];
                score[nonbidderTeam]+=score[bidderTeam];
                score[bidderTeam]=0;
                if(redouble>0&&capot==0)
                {
                    score[nonbidderTeam]-=(anno[nonbidderTeam]);
                    score[nonbidderTeam]*=(redouble*2);
                }
                score[nonbidderTeam]+=(anno[bidderTeam]);
                realScore[nonbidderTeam]+=hang;
                hang=0;
                wasThereHanging=0;
        }
        if(redouble>0&&score[bidderTeam]>score[nonbidderTeam]&&capot==0)
        {
            score[nonbidderTeam]-=anno[nonbidderTeam];
        ...
        {
            wasThereHanging=1;
            hang2=score[bidderTeam];
            score[bidderTeam]=0;
        }
        if(score[nonbidderTeam]%10==6&&bid%10<5){
            score[nonbidderTeam]++;
            score[bidderTeam]--;
        }
        if(score[nonbidderTeam]%10==4&&bid%10==6){
            score[nonbidderTeam]++;
            score[bidderTeam]--;
        }
        if(bid%10<5)
        {
            ...
            if(wasThereHanging==1)hang2=hang2/10;
        }
        if(bid%10==5)
        {
            score[0]*=2;
            score[1]*=2;
            if(score[0]%10<5) realScore[0]+=(score[0]/10);
            else realScore[0]+=((score[0]/10)+1);
            if(score[1]%10<5) realScore[1]+=(score[1]/10);
            else realScore[1]+=((score[1]/10)+1);
            if(wasThereHanging==1)
            hang2=13;
        }
        if(bid%10==6)
        {
            ...
            if(wasThereHanging==1)hang2=(hang2/10)+1;
        }
        hang+=hang2;
        cout<<realScore[0]<<" "<<realScore[1]<<" "<<"hang : "<<hang<<endl;
        score[0]=129;
        score[1]=129;
        bid=6;
        anno[0]=0;
